	...
	<div>
		<div class="breadcrumb__container">
			<el-breadcrumb separator="/">
				...
				<el-breadcrumb-item>{{ $t('Animaux à l’adoption')}}</el-breadcrumb-item>
			</el-breadcrumb>
		</div>
		...
		<div class="containerRouterLeft">
			<div class="blocTitle">
				<div class="blocTitle__wrapperTitle">
					<h2 class="blocTitle__title">{{$t('List of animals for adoption')}}</h2>
					<span class="blocTitle__count"><strong>{{ animals.total }}</strong> {{$t('results')}}</span>
				</div>
				<div class="blocTitle__wrapperBtns">
					<!-- If user is on a paid plan or on trial -->
					<el-button type="primary"
										 v-if="laravel.canUseFullApp || animals.total <= laravel.maxFreeAnimals"
										 @click="dialogCreateAnimalAdoption = true"
										 icon="el-icon-plus">
						{{ $t('Add a animal')}}
					</el-button>
					<!-- Else if user isn't on a paid plan and has an total of animal <= 25 -->
					<el-tooltip v-else-if="laravel.canUseFullApp == false && animals.total >= laravel.maxFreeAnimals"
											content="Vous avez atteint le quota maximum d’animaux autorisés (25) avec le plan gratuit."
											placement="top">
						<span>
							<el-button type="primary" disabled icon="el-icon-plus">{{ $t('Add a animal')}}</el-button>
						</span>
					</el-tooltip>
				</div>
			</div>
			...
			<el-row v-if="types.length >= 0 && races.length >= 0 && colors.length >= 0" style="margin-top: 20px;">
				<el-col :span="3" v-if="types.length > 1" style="margin-right: 10px; min-width: 130px;">
					<el-select clearable
										 style="width: 100%"
										 placeholder="Type"
				...
				<el-col :span="3" v-if="races.length > 1" style="margin-right: 10px; min-width: 130px;">
					<el-select clearable
										 style="width: 100%"
										 placeholder="Race"
				...
				<el-col :span="3" style="min-width: 130px" v-if="colors.length > 1">
					<el-select clearable
										 style="width: 100%"
										 placeholder="Couleur"
				...
				<el-button icon="el-icon-delete" circle
									 size="mini" @click="resetFilters"
									 class="el-button__resetFilters"
									 v-show="filterByType > 0 || filterByRace > 0 || filterByColor > 0"></el-button>
				<el-col :span="4" style="float: right; min-width: 210px;">
					<el-input
									placeholder="Chercher un animal"
									suffix-icon="el-icon-search"
									clearable
									@input="debounceGetAnimals"
									v-model="keywords">
					</el-input>
				</el-col>
			</el-row>
			...
			<el-table :data="animals.data"
				...
				<!--
				<el-table-column prop="date_of_birth"
												 :formatter="filterDateLocale"
				...
				<el-table-column label="Opérations" width="120px">
					<template slot-scope="scope">
						<router-link :to="{ name: 'AdoptionView', params: { id: scope.row.id } }">
		...
		<!-- Create an pet -->
		<el-dialog v-if="laravel.canUseFullApp || animals.total <= laravel.maxFreeAnimals" :visible.sync="dialogCreateAnimalAdoption"
							 custom-class="createAnimal"
							 :lock-scroll="true">
			
			<span slot="title" class="el-dialog__title">
				{{$t('Create an animal for adoption')}}
			</span>
			...
			<el-form label-position="top" :model="createForm">
				...
				<el-row :gutter="20">
					...
					<el-col :span="12">
						<el-form-item label="temp"
													required
													prop="id_number"
													:rules="{ required: true, message: this.$t('validation.required', {attribute: this.$t('Dashboard')}), trigger: ['blur'] }"
				...
				<el-row :gutter="20" v-if="createForm.dialogMoreFieldsIsVisible == true">
				...
				<el-button
								v-if="createForm.dialogMoreFieldsIsVisible == false"
								type="text"
								size="small"
								:disabled="createForm.busy"
								@click="createForm.dialogMoreFieldsIsVisible = true">
...
<script>
    ...
    export default {
        props: ['currentTeam', 'teams', 'billableType'],
        ...
        data() {
            return {
                ...
                createForm: new SparkForm({
                    ...
                    race_id: '',
                    color_id: '',
                    dialogMoreFieldsIsVisible: false
                }),
                types: [],
                races: [],
                colors: [],
                ...
                loading: true,
                errored: false,
                dialogCreateAnimalAdoption: false,
                filterByType: "",
                filterByRace: "",
                filterByColor: "",
                keywords: ""
        ...
        mounted() {
            ...
            this.getAnimals();
            this.$on('updateListAnimalsAdoption', () => {
                self.getAnimals()
            })
        },
        ...
        methods: {
            ...
            debounceGetAnimals: debounce(function () {
                this.getAnimals();
            }, 300),
            ...
            store() {
                ...
                Spark.post('animals/adoptions', this.createForm)
                    .then(response => {
                        this.dialogCreateAnimalAdoption = false;
                        this.$message({message: 'L’animal a été correctement ajouté.', type: 'success'});
                        this.$emit('updateListAnimalsAdoption');
            ...
            handleCurrentChange(val) {
                this.loading = true;
                axios.get(`animals/adoptions?page=${val}&color=${this.filterByColor}&race=${this.filterByRace}&type=${this.filterByType}&keywords=${this.keywords}`)
                    .then(response => {
                        this.animals = response.data
                    })
                    .catch(error => {
